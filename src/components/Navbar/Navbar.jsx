...
const Navbar = () => {
  ...
  return (
    <header className="fixed top-0 left-0 w-full h-[15vh] p-8 z-50 bg-transparent">
      <nav className="w-full lg:w-[90%] flex justify-between items-center h-[60px] py-2 md:px-6 lg:px-20 mx-auto">
        
        {/* Logo Section */}
        <div className="flex items-center bg-[#FACC15] rounded-[50%] md:rounded-full md:px-4 cursor-pointer">
          <NavLink to="/" className="flex items-center">
            <img src={logo} alt="brandlogo" className="w-12 h-12 object-contain rounded-full" />
            <h1 className="text-white hidden md:block text-lg font-bold">NdimAfrika</h1>
          </NavLink>
        </div>
      ...
      {/* Mobile Navigation Menu */}
      <div className={`fixed inset-0 h-[100vh] bg-black text-white flex flex-col items-start justify-start transition-transform duration-500 ease-in-out ${
          isOpen ? "translate-x-0" : "-translate-x-full"
        }`}
      >
        <button className="absolute top-6 right-6 text-4xl" onClick={() => setIsOpen(false)}>
        ...
        <ul className="flex flex-col w-full mt-20 pl-6 space-y-8 text-2xl font-semibold">
          <li className="border-b border-gray-700 pb-2 last:border-b-0">
            <NavLink 
              to="/" 
              className={({ isActive }) => isActive ? "text-yellow-500 font-bold" : "text-white"} 
              onClick={() => setIsOpen(false)}
            >
              Explore Library
            </NavLink>
